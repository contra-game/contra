...
Сеть ещё не подключена. Выбран Photon Fusion Godot SDK 3.0 — GDExtension-аддон
с API прямо из GDScript (`Fusion.connect_to_photon`, `FusionSpawner`,
`Fusion.rpc`). Важно: это **preview-версия**, Photon сам пишет, что она не для
продакшена.

Чтобы подключить:

1. Скачать SDK со страницы
   [Fusion Godot SDK & Download](https://doc.photonengine.com/fusion-godot/v3-shared-authority/getting-started/sdk-download)
   — нужен вход в аккаунт Photon, поэтому шаг ручной.
2. Распаковать папку `fusion/` в `addons/` этого проекта и переоткрыть его.
3. Скопировать `photon.cfg.example` в `photon.cfg` и вписать свой App ID
   (тип приложения — Fusion, создаётся в Photon Dashboard). `photon.cfg`
   в git не попадает: репозиторий публичный.

Код к этому готов: урон, спавны и ИИ уже отделены проверками
`multiplayer.is_server()`, а `scripts/net_config.gd` сообщает, чего не хватает.
